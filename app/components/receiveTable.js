...
import { getReports, updateReport} from "../lib/helper";
import { useQuery } from "@tanstack/react-query";
import { useState } from "react";

export default function ReceiveTable({ userData }) {

    const { department, role } = userData;
    // console.log(user_id, department, role);// ✅ ตรวจสอบค่า userData ที่ส่งมา

    const { isLoading, isError, data, error } = useQuery({queryKey: ['reports', userData], queryFn:() => getReports(userData)})

    // console.log("data in ReceiveTable:", data); // ✅ ตรวจสอบค่า data ที่ได้จาก useQuery
...
function Tr({report_id, topic, description, image, status, department }) {
        ...
        <tr className="bg-gray-100 text-center">
            ...
            <td className="px-4 py-2 border-b columns-2">
                <div className="flex justify-center items-center">
                    <button onClick={() => updateReport(reportId, { topic, description, image, status, department })}><TbEdit color="green" size={20} /></button>
                </div>
                <div className="flex justify-center items-center">
